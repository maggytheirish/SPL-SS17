...
list.of.packages = c("rpart","lubridate","outliers","rpart.plot", "xgboost",
                     "caret","caretEnsemble", "randomForest","e1071","pROC","tidyr",
                     "klaR","car","devtools","yamldebugger","mlbench","Hmisc", 
                     "ggvis","relaimpo", "formatR","data.table", "zoo", "ggplot2",
                     "forecast", "reshape2", "pdp")
...
train$Promo2SinceDate = as.Date(with(train, paste(Promo2SinceYear,Promo2SinceWeek,1, 
                                                  sep="-")),"%Y-%U-%u")



# Creating a starting date for Competition (assuming it's from the 
#                                           first day of the month)
train$CompetitionSinceDate = as.Date(paste(train$CompetitionOpenSinceYear, 
                                           train$CompetitionOpenSinceMonth, 1, sep="-"),
                                           format="%Y-%m-%d")

# Replacing NAs in CompetitionSinceDate
CompetitionDateMissing <- is.na(train$CompetitionSinceDate)==T & 
                                !is.na(train$CompetitionDistance)==T
...
factor.vars = c("StoreType", "Assortment", "Promo2","DayOfWeek", "Open", "Promo", "StateHoliday", "SchoolHoliday", "PromoInterval", "StoreAssortmentMatch", "BothHolidays")
