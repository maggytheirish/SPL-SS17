...
avgsalesperstore <- aggregate(train$Sales, list(train$Store), mean)
avgvisitsperstore <- aggregate(train$Customers, list(train$Store), mean)

avgsalesperstoreperdayofweek <- aggregate(train$Sales, list(train$Store,train$DayOfWeek), mean)
avgvisitsperstoreperdayofweek <- aggregate(train$Customers, list(train$Store,train$DayOfWeek), mean)

avgsalesperstoreperyear <- aggregate(train$Sales, list(train$Store,train$Year), mean)
avgvisitsperstoreperyear <- aggregate(train$Customers, list(train$Store,train$Year), mean)

avgsalesperstorepermonth <- aggregate(train$Sales, list(train$Store,train$Year,train$Month), mean)
avgvisitsperstorepermonth <- aggregate(train$Customers, list(train$Store,train$Month), mean)

```

```{r}
names(avgsalesperstore)[1]<-"Store"
names(avgsalesperstore)[2]<-"AvgSalesPerStore"
train<-merge(train, avgsalesperstore, by="Store")
```

```{r}
names(avgvisitsperstore)[1]<-"Store"
names(avgvisitsperstore)[2]<-"AvgVisitsPerStore"
train<-merge(train, avgvisitsperstore, by="Store")
```
