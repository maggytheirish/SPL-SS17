...
```{r}
sales_store <- aggregate(train$Sales,by = list(train$Store,train$Promo),mean)
names(sales_store ) <- c("Store","Promo","AvgSalesbyStore")
ggplot(data=sales_store ,aes(Store,AvgSalesbyStore, color=Promo, shape =Promo)) + geom_point() +
...
ggtitle("Log of Average Sales of each store by log of competition distance")
```
